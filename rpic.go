...
const (
	ERR_CONF_EXISTS              = "config file exists and will not be overwritten"
	ERR_CONFIG_NOT_FOUND         = "config file not found"
	ERR_DATABASE_CONNECTION      = "unable to connect to database"
	ERR_DATABASE_NOT_INITIALIZED = "database has not been initialized"
	ERR_EMPTY_USER_NAME          = "user query cannot search for an empty name"
	ERR_EMPTY_USER_TOKEN         = "user query cannot search for an empty token"
	ERR_INVALID_USER_NAME        = "invalid user name"
	ERR_USER_INVALID             = "inavlid user"
	ERR_INVALID_PASSWORD         = "invalid password"
	ERR_USER_ADMIN_NOT_EXIST     = "admin user does not exist"
)
